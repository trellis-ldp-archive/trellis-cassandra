...
public class CassandraBinaryService implements BinaryService {
	...
	public Optional<InputStream> getContent(IRI identifier, List<Range<Integer>> ranges) {
		...
		try {
			if (ranges.isEmpty()) {
				return Optional.ofNullable(readAll(identifier));
			} else {
				final List<InputStream> iss = new ArrayList<>();
				for (final Range<Integer> r : ranges) {
					InputStream input = readRange(identifier, r);
					iss.add(input);
				}
				return Optional.of(new SequenceInputStream(asEnumeration(iss.iterator())));
			}
	...
	private InputStream readRange(IRI identifier, Range<Integer> range) throws IOException {
